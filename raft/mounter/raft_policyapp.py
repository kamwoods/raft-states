#!/usr/bin/env python3
#
# RAFT Mounter and Read/Write Application Indicator
#
# adapted from the:
...
class PolicyDialog(gtk.Dialog):
    ...
    def __init__(self, dialog_message):
        gtk.Dialog.__init__(self, "USB Device Mount Policy", None, 0)
...
class MounterAppIndicator:
	...
	# Read-write status
        #if os.path.isfile("/etc/udev/rules.d/fstab.rules"):
        rwstatus = gtk.MenuItem.new_with_label("Set USB mount policy READ-WRITE")
    ...
    def ro_set(self, widget, data=None):
        ...
        if not os.path.isfile("/etc/udev/rules.d/fstab.rules"):
            win = PolicyDialog("Click OK to set system-wide USB mount policy to:" \
                           + "\n\n" + "READ-ONLY" + "\n\n" + \
    ...
    def rw_set(self, widget, data=None):
        ...
        if os.path.isfile("/etc/udev/rules.d/fstab.rules"):
            win = PolicyDialog("Click OK to set the system-wide USB mount policy to:" \
                           + "\n\n" + "WRITEABLE" + "\n\n" + \
